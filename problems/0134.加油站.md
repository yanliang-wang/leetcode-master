...
* 情况三：如果累加的最小值是负数，汽车就要从非0节点出发，从后向前，看哪个节点能这个负数填平，能把这个负数填平的节点就是出发节点。(==倒着来，看从哪里可以把这个最小的负值填满，那么这个地方就是可以开始的地方那个==)
...
public:
    int canCompleteCircuit(vector<int>& gas, vector<int>& cost) {
        ...
        if (min >= 0) return 0;     // 情况2
                                    // 情况3
        for (int i = gas.size() - 1; i >= 0; i--) {	// 倒着来，看从哪里可以把这个最小的负值填满，那么这个地方就是可以开始的地方那个
            int rest = gas[i] - cost[i];
            min += rest;
            if (min >= 0) {
                return i;
