...
> 第一次接触背包问题，觉得难点有两个
>
> 一是如何把这个问题和标准的背包问题链接到一起，概念的对应，比如背包是谁，重量代表什么，价值代表什么
>
> 二是找到dp元素对应的意义
>
> ​	**套到本题，dp[j]表示 背包总容量是j时，使用数组元素凑出的最大的小于或等于j的总和，所以dp[i] <= i**。
...
**套到本题，dp[j]表示 背包总容量是j，使用数组元素凑出的最大的小于或等于j的总和，所以dp[i] <= i**。
...
public:
    bool canPartition(vector<int>& nums) {
        ...
        for(int i = 0; i < nums.size(); i++) {
            for(int j = target; j >= nums[i]; j--) { // 每一个元素一定是不可重复放入，所以从大到小遍历
                dp[j] = max(dp[j], dp[j - nums[i]] + nums[i]); // dp[j - nums[i]] 
            }
        }
